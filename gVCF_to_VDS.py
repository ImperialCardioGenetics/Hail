...
def main():
    ...
    p.add_argument('--sample-qc-out',  type=str, required=True,
                   help="Where to write the sample_qc Hail Table")
    ...
    # sample QC
    vds = hl.vds.read_vds(args.output_vds)
    qc = hl.vds.sample_qc(vds, dp_bins=(0,10,20,30,40,99), gq_bins=(0,10,20,30,40,99))
    qc.write(args.sample_qc_out, overwrite=True)
    print("Wrote sample QC to", args.sample_qc_out)
